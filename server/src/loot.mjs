...
function pickName(list, rng, fallbackList) {
  const pool = Array.isArray(list) && list.length ? list : fallbackList;
  if (!Array.isArray(pool) || !pool.length) return null;
  return pool[randInt(rng, 0, pool.length - 1)];
}

function nameFor(rarity, baseName, rng, flavor = null) {
  const weighed = flavor === "avarice";
  ...
  if (rarity === "unique") {
    return (
      pickName(weighed ? affixes.avarice_unique_names : null, rng, affixes.unique_names) ||
      baseName
    );
  }
  if (rarity === "canto_unique") {
    return (
      pickName(
        weighed ? affixes.avarice_canto_unique_names : null,
        rng,
        affixes.canto_unique_names
      ) || baseName
    );
  }
  return baseName;
...
function makeItem(rarity, itemPool, soulbound, seed, flavor = null) {
  ...
  return {
    ...
    name: nameFor(rarity, base.name, rng, flavor),
    rarity,
    itemPool,
    seed,
...
function rollOnce(table, rng, { champion = false, boss = false, flavor = null } = {}) {
  const items = [];
  ...
  for (let i = 0; i < rolls; i++) {
    ...
    items.push(makeItem(pick.rarity, pick.item_pool, pick.soulbound, newSeed(), flavor));
  }
  if (boss && Array.isArray(table.boss_guaranteed)) {
    for (const g of table.boss_guaranteed) {
      items.push(makeItem(g.rarity, g.item_pool, g.soulbound, newSeed(), flavor));
    }
  }
  return items;
}
...
export function rollDrops(dropTableId, opts = {}) {
  ...
  const flavor =
    opts.flavor ||
    (String(dropTableId || "").includes("avarice") || String(table?.id || "").includes("avarice")
      ? "avarice"
      : null);
  return rollOnce(table, rng, { ...opts, flavor });
}
