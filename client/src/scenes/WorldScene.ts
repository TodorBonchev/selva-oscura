...
export class WorldScene extends Phaser.Scene {
      ...
      if (hit) {
        ...
        if (hit.kind === "poi" || hit.kind === "exit") {
          ...
          if (hit.kind === "exit" && hit.toCanto) {
            // Also request travel after interact toast
  ...
  pickEntity(sx: number, sy: number): any | null {
    ...
    const consider = (e: any) => {
      ...
      const d = Phaser.Math.Distance.Between(sx, sy, p.sx, p.sy);
      if (d < bestD) {
        bestD = d;
        best = e;
      }
  ...
  redraw() {
          ...
          e.poiKind === "ah"
            ...
            : e.poiKind === "stash"
              ...
              : e.poiKind === "quest"
                ...
                : 0xd7e0d4;
        g.fillStyle(col, 1);
        g.fillCircle(p.sx, p.sy - 10, 8);
        this.addLabel(p.sx, p.sy - 28, e.label || e.name);
      } else if (e.kind === "exit") {
        g.fillStyle(0x88aaff, 0.9);
        g.fillTriangle(p.sx, p.sy - 16, p.sx + 10, p.sy, p.sx - 10, p.sy);
        this.addLabel(p.sx, p.sy - 28, e.label || "Exit");
      } else if (e.kind === "mob") {
    ...
    for (const pl of this.room.players) {
      ...
      this.addLabel(p.sx, p.sy - 28, isYou ? "You" : pl.name);
    ...
    if (Phaser.Input.Keyboard.JustDown(this.keys.E)) {
      ...
      if (best) {
        if (best.kind === "loot") this.socket.pickup(best.id);
          ...
          this.socket.interact(best.id);
          if (best.toCanto) this.time.delayedCall(50, () => this.socket.travel(best.toCanto));
        }
      }
    }
  }
